...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss,cls_loss = base_model(points)
            try:
                _loss = loss + 0.1 * cls_loss
                _loss.backward()
                # print("Using one GPU")
            except:
                loss = loss.mean()
            ...
            if args.distributed:
                ...
                losses.update([loss.item()*1000,cls_loss.item()])
            ...
            if train_writer is not None:
                ...
                train_writer.add_scalar('Loss/Batch/clsLoss', cls_loss.item(), n_itr)
