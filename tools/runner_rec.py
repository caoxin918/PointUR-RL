...
def runner_rec(args, config, train_writer=None, val_writer=None):
    ...
    # resume ckpts实现的功能是根据命令行参数来恢复模型或者加载预训练模型。
    if args.resume:
    ...
    if args.distributed:
        ...
        #使用PyTorch库中的DataParallel模块将base_model模型转换为可以在多个GPU上并行运行的模型。
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses = AverageMeter(['Loss','cls_loss'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss,emd_loss, = base_model(points)
            try:
                _loss = loss
                _loss.backward()
                # print("Using one GPU")
            ...
            if args.distributed:
                ...
                losses.update([loss.item()*1000,emd_loss])
            ...
            if train_writer is not None:
                ...
                train_writer.add_scalar('Loss/Batch/emd_loss', emd_loss, n_itr)
